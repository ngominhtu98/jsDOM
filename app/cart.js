...
function updateCartotal() {
    ...
    Array.from(cartRows).forEach(function(cartRow) {
        ...
        var check = cartRow.querySelector(".check")
        if(check.checked == true){
            totalCheck = totalCheck + (quantity * price);
        };
        check.addEventListener("click", updateCartotal)


        total = total + (quantity * price);

    });
    document.querySelector(".totalPrice").innerText = formatNumber(totalCheck);
